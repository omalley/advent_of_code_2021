...
pub fn generator(data: &str) -> Vec<Display> {
  data.lines()
    .map(|x| x.trim())
    .filter(|x| x.len() > 0)
...
pub fn part1(lines: &Vec<Display>) -> i32 {
  lines.iter().map(|x| x.easy_numbers()).sum()
}

pub fn part2(lines: &Vec<Display>) -> i32 {
  lines.iter().map(|x| x.unscramble()).sum()
}
